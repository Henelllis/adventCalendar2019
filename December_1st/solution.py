...
def requiredFuel(mass):
    divided_mass = mass / 3
    floored_divided_mass = math.floor(divided_mass)
    mass_sub_2 = floored_divided_mass - 2
    return mass_sub_2

def part1():
    for i in range(len(lines)):
        lines[i] = requiredFuel(lines[i])
...
divide by three, round down, and subtract 2. However, that fuel also requires fuel, 
...
def recursive_fuel(mass_fuel):

    fuel_counter = 0
    while(mass_fuel >= 0):
        og_mass_fuel = mass_fuel
        mass_fuel = requiredFuel(mass_fuel)
        if(mass_fuel >= 0):
            fuel_counter += mass_fuel
        print('\t\t og mass value {} after transform {} fuel Counter {}'.format(og_mass_fuel,mass_fuel,fuel_counter))
...
def part2():
    for i in range(len(lines)):
        og_value = lines[i]
        lines[i] = recursive_fuel(lines[i])
        print('before transform {} after transform {}'.format(og_value, lines[i]))
    print(sum([  line for line in lines]))
